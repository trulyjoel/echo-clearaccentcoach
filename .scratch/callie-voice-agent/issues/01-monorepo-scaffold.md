...
`apps/web` is now deployed to Vercel at https://callie-livid.vercel.app (project already linked as
`holmes-tech/callie`, root directory `apps/web`). Set `VITE_CLERK_PUBLISHABLE_KEY` and `VITE_API_URL`
(`https://callie-server.fly.dev`) as production env vars, then `vercel --prod` from the repo root (Vercel's
configured root directory made running from inside `apps/web` fail — it resolved to `apps/web/apps/web`).

Also updated the `callie-server` Fly app's `WEB_ORIGIN` secret from the `localhost:5173` placeholder to the
